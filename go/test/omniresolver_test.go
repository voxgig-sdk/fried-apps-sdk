...
import (
	...
	omni "github.com/voxgig-sdk/fried-apps-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.FriedAppsError); is {
				out := map[string]any{
					"name":    "FriedAppsError",
...
func omniCtx(arg any, client *sdk.FriedAppsSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.FriedAppsSDK); is {
			client = live
