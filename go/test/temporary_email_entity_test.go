...
import (
	...
	"github.com/voxgig-sdk/fried-apps-sdk/go/core"

	vs "github.com/voxgig-sdk/fried-apps-sdk/go/utility/struct"
)
...
func TestTemporaryEmailEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.TemporaryEmail(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.TemporaryEmail(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set FRIED_APPS_TEST_TEMPORARY_EMAIL_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		temporaryEmailRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.temporary_email")))
		...
		temporaryEmailRef01Match := map[string]any{}

		temporaryEmailRef01ListResult, err := temporaryEmailRef01Ent.List(temporaryEmailRef01Match, nil)
		if err != nil {
			t.Fatalf("list failed: %v", err)
		}
		_, temporaryEmailRef01ListOk := temporaryEmailRef01ListResult.([]any)
		if !temporaryEmailRef01ListOk {
			t.Fatalf("expected list result to be an array, got %T", temporaryEmailRef01ListResult)
		}
...
func temporary_emailBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("FRIED_APPS_TEST_TEMPORARY_EMAIL_ENTID")
	...
	idmapResolved := core.ToMapAny(env["FRIED_APPS_TEST_TEMPORARY_EMAIL_ENTID"])
	...
	if env["FRIED_APPS_TEST_LIVE"] == "TRUE" {
		...
		client = sdk.NewFriedAppsSDK(core.ToMapAny(mergedOpts))
	}
