...
[MCP](https://modelcontextprotocol.io) server exposing the FriedApps SDK as
two agent tools — `fried-apps_list` and `fried-apps_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/fried-apps-mcp)
make build

# 2. Provide credentials via the environment
export FRIED_APPS_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user fried-apps \
  -- /absolute/path/to/fried-apps-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./fried-apps-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// fried-apps_list: first page of records
{ "entity": "temporary_email" }
{ "entity": "temporary_email", "query": { } }

// fried-apps_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/fried-apps-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user fried-apps \
     -- "$PWD"/dist/*/fried-apps-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `fried-apps_list` and `fried-apps_load` tools now appear
   in new sessions. Ask the agent to *"list temporary_email using fried-apps"*
   and it calls `fried-apps_list` with `{"entity":"temporary_email"}`.
...
```sh
export FRIED_APPS_APIKEY=sk_live_xxx            # API key
export FRIED_APPS_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `fried-apps_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `fried-apps_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `FRIED_APPS_APIKEY` | API key sent with every request. |
| `FRIED_APPS_BASE` | Optional override of the API base URL. |
...
```sh
./fried-apps-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"fried-apps_load","arguments":{"entity":"temporary_email","query":{"id":1}}}}'
```
