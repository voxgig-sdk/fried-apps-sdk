...
Fried Apps API client, generated from the OpenAPI spec.

The Fried Apps API allows users to generate temporary email addresses with just one click.

Learn more about Fried Apps API at [github.com/l0v3m0n3y/friedapps](https://github.com/l0v3m0n3y/friedapps).

This is an unofficial SDK for the Fried Apps public API, generated by Voxgig with [`@voxgig/sdkgen`](https://github.com/voxgig/sdkgen). It is not affiliated with, endorsed by, or sponsored by the upstream API provider.
...
const items = await client.TemporaryEmail().list()
```
...
```go
client := sdk.Test()
result, err := client.TemporaryEmail(nil).List(
    nil, nil,
)
```
...
```bash
cd go-mcp && go build -o fried-apps-mcp .
```
...
```json
{
  "mcpServers": {
    "fried-apps": {
      "command": "/abs/path/to/fried-apps-mcp"
    }
  }
}
```
...
| --- | --- | --- |
| **TemporaryEmail** | The TemporaryEmail entity (list). | `/temp-mail/domains` |
...
# List all temporaryemails (returns a list, raises on error)
temporaryemails = client.TemporaryEmail().list()
...
// List all temporaryemails (returns an array; throws on error)
$temporaryemails = $client->TemporaryEmail()->list();
print_r($temporaryemails);
```

### Golang

```go
import sdk "github.com/voxgig-sdk/fried-apps-sdk/go"

client := sdk.New()

// List all temporaryemails
temporaryEmails, err := client.TemporaryEmail(nil).List(nil, nil)
if err != nil {
    panic(err)
}
fmt.Println(temporaryEmails)
```
...
# List all temporaryemails (returns an Array; raises on error)
temporaryemails = client.TemporaryEmail.list
puts temporaryemails
```
...
Generated from the Fried Apps API OpenAPI spec by [@voxgig/sdkgen](https://github.com/voxgig/sdkgen).
