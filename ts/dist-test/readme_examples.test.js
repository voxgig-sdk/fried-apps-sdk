...
function isRunnable(code) {
    return (/new\s+FriedAppsSDK\b/.test(code) ||
        /\bFriedAppsSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+FriedAppsSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/FriedAppsSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
