# FriedApps TypeScript SDK



The TypeScript SDK for the FriedApps API — a type-safe, entity-oriented client with full async/await support.

The API is exposed as capitalised, semantic **Entities** — e.g.
`client.TemporaryEmail()` — each with a small set of operations (`list`)
...
| `tester(testopts?, sdkopts?)` | `FriedAppsSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `FriedAppsSDK.test(testopts?, sdkopts?)` | `FriedAppsSDK` | Create a test-mode client. |
...
| `client` | `client(): FriedAppsSDK` | Return the parent SDK client. |
...
```ts
const temporary_emails = await client.TemporaryEmail().list()
```
...
fried-apps/
├── src/
│   ├── FriedAppsSDK.ts        # Main SDK class
