// Command FriedApps-cli is a CLI + REPL for the FriedApps SDK,
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      FRIED_APPS_APIKEY (api key), FRIED_APPS_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: FRIED_APPS_APIKEY carries the API key and
	// FRIED_APPS_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("FRIED_APPS_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("FRIED_APPS_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewFriedAppsSDK(opts)
