...
boru-driven command-line client **and** interactive REPL for the FriedApps
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/fried-apps-cli)
make build
...
./fried-apps-cli list temporary_email

# 5. Override the API base URL for a single call
FRIED_APPS_BASE=https://api.example.com ./fried-apps-cli list temporary_email

# 6. No arguments -> interactive REPL
./fried-apps-cli
fried-apps> list temporary_email
fried-apps> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/fried-apps-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/fried-apps-cli list temporary_email
   ```
...
Run with no arguments to open a REPL (prompt `fried-apps>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/fried-apps-cli`. |
...
(`./fried-apps-cli <expr>`) and interactively (the REPL), and expressions compose the
