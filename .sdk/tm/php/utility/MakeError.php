...
class FriedAppsMakeError
{
    public static function call(?FriedAppsContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new FriedAppsContext([], null);
        }
        $op = $ctx->op ?? new FriedAppsOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new FriedAppsResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof FriedAppsError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new FriedAppsError('', $msg, $ctx);
        ...
        if ($err instanceof FriedAppsError) {
