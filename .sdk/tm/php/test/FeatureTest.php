...
class FtCtrl extends FriedAppsControl
{
...
class FtClient
{
    ...
    private FriedAppsUtility $utility;

    public function __construct(FriedAppsUtility $utility, array $options)
    {
    ...
    public function get_utility(): FriedAppsUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (FriedAppsContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public FriedAppsContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (FriedAppsContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = FriedAppsConfig::make_feature($name);
    ...
    private static function buildUrl(FriedAppsSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof FriedAppsError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new FriedAppsResponse($fetched);
        }
    ...
    private function fail(FriedAppsContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (FriedAppsContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
