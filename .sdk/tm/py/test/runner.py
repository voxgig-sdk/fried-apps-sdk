...
class FriedAppsTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            FriedAppsTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = FriedAppsTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = FriedAppsTestRunner.getenv("PROJECTENV_TEST_LIVE")
        override = FriedAppsTestRunner.getenv("PROJECTENV_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = FriedAppsTestRunner.getenv(key)
        ...
        explain = FriedAppsTestRunner.getenv("PROJECTENV_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if FriedAppsTestRunner._test_control is not None:
            return FriedAppsTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                FriedAppsTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            FriedAppsTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = FriedAppsTestRunner.load_test_control()
...
def load_env_local():
    FriedAppsTestRunner.load_env_local()


def env_override(m):
    return FriedAppsTestRunner.env_override(m)


def entity_data(v):
    return FriedAppsTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return FriedAppsTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return FriedAppsTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return FriedAppsTestRunner.load_test_control()


def live_client_options():
    return FriedAppsTestRunner.live_client_options()


def live_delay_ms():
    return FriedAppsTestRunner.live_delay_ms()
