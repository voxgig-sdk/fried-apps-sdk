...
module FriedAppsUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = FriedAppsContext.new({}, nil)
    end
    op = ctx.op || FriedAppsOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || FriedAppsResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(FriedAppsError) ? err.msg : err.to_s
    ...
    sdk_err = FriedAppsError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(FriedAppsError)
