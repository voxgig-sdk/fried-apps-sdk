...
// FriedApps SDK TemporaryEmail entity
...
class TemporaryEmailEntity
{
    ...
    {
        if ($args) {
            $this->_data = FriedAppsHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = FriedAppsHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    /**
     * List TemporaryEmail items matching the given filter.
     *
     * @param TemporaryEmailListMatch|array|null $reqmatch Match filter (any subset
     *   of TemporaryEmail fields) as an assoc-array; TemporaryEmailListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return TemporaryEmail[]|array A list of TemporaryEmail items as assoc-arrays at
     *   the SDK boundary; throws FriedAppsError on failure (item-5 convention).
     */
