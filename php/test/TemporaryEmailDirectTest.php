...
{
    public function test_direct_list_temporary_email(): void
    {
        $setup = temporary_email_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-temporary_email", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
...
function temporary_email_direct_setup($mockres)
{
    ...
    if ($live) {
        ...
        $client = new FriedAppsSDK($merged_opts);
        return [
            "client" => $client,
            "calls" => $calls,
            "live" => true,
