...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = FriedAppsOmni.make_runner(TEST_JSON_FILE, FriedAppsSDK.test(nil, nil))
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : FriedAppsOmni.errify(err)
    end
