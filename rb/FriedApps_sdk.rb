...
require_relative 'FriedApps_types'
...
class FriedAppsSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = FriedAppsHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = FriedAppsHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, FriedAppsFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    FriedAppsUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = FriedAppsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = FriedAppsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = FriedAppsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => FriedAppsError.new(
        "#{op}_allow",
        "FriedAppsSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = FriedAppsHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = FriedAppsError.new(
        "graphql_error", "FriedAppsSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
