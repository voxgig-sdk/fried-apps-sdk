...
module FriedAppsUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = FriedAppsResponse.new({})
    ...
    if fetch_err
      ...
      response = FriedAppsResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = FriedAppsResponse.new(fetched)
    else
