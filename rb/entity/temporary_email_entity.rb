...
class TemporaryEmailEntity
  ...
  def data_set(args)
    if args
      @_data = FriedAppsHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = FriedAppsHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #
  # @param reqmatch [TemporaryEmailListMatch, Hash, nil] match filter (any subset of
  #   TemporaryEmail fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<TemporaryEmail>, Array] the matching TemporaryEmail items; raises FriedAppsError on failure
